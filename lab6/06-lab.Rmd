...
instituions.unis <- as.data.frame(table(institution))
head(instituions.unis)
```
...
schools_and_deps <- stringr::str_extract_all(
  publications_txt,
  "(School of \\w+)|(Department of [\\w\\s]+)"
  )
schools.depts <- as.data.frame(table(schools_and_deps))
head(schools.depts)
```
...
abstracts <- str_remove_all(abstracts, "<AbstractText>|</AbstractText>")
abstracts1 <- str_remove_all(abstracts, "\\s+")
...
```{r}
sum(is.na(abstracts1))

```

_Answer here._

There are 188 that do not have an abstract. 
...
There are none that do not have a title. 
...
db <- knitr::kable(database)
kableExtra::scroll_box(db, width = "800px", height = "1200px")
